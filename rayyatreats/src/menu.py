...
def load_products() -> list["Product"]:
    """Load products from the runtime snapshot (data/products.json).

    The snapshot is regenerated by ``sync.refresh()`` on every run (it falls
    back to the committed baseline when the live fetch yields nothing), so an
    empty or missing file means refresh has not run and is treated as a fatal
    error rather than letting the empty list crash the menu later.
    """
    if not DATA_FILE.exists():
        raise RuntimeError(f"找不到商品資料檔：{DATA_FILE}，請先執行 refresh")
    raw = json.loads(DATA_FILE.read_text(encoding="utf-8")).get("products", [])
    if not raw:
        raise RuntimeError(f"商品資料檔無任何商品：{DATA_FILE}，請先執行 refresh")
    return [
        Product(
            handle=p["handle"],
            name=p["name"],
